...
function drawA2(ctx, canvasX, canvasY, canvasWidth, canvasHeight) {
  ...
  if (A2Phase === "lap") {
    const prevDist = A2LapProgress * totalLen;
    A2LapProgress += speed / totalLen;
    if (A2LapProgress >= 1) A2LapProgress = 1;
    ...
    const na = a + turn;
    A2VelX = Math.cos(na) * speed;
    A2VelY = Math.sin(na) * speed;
    ...
    if (bounced) {
      ...
      A2VelX = Math.cos(ba) * speed;
      A2VelY = Math.sin(ba) * speed;
    }
  ...
  for (let i = 1; i < A2Trail.length; i++) {
    ...
    const p1 = A2Trail[i];
    const age = now - p1.t;
    const alpha = A2_TRAIL_FADE_ENABLED
      ? Math.max(0, Math.min(1, 1 - age / A2_TRAIL_FADE_MS))
      : 1;
    if (alpha <= 0) continue;
